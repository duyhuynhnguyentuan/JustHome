//
//  StepSixOTPVerificationViewModel.swift
//  JustHome
//
...
class StepSixOTPVerificationViewModel: ObservableObject {
    var nowDate = Date()
    ...
    @MainActor
    func stepSixSendOTP() async throws {
        do {
            _ = try await contractsService.loadStepSixSendOTP(by: contractId)
    ...
    @MainActor
    func stepSixVerifyOTP() async throws -> MessageResponse {
        defer { loadingState = .finished }
        do {
            loadingState = .loading
            let response = try await contractsService.stepSixVerifyOTP(by: contractId, otp: verificationCode)
            return response
        }catch let error as NetworkError {
            self.error = error
            print("Error: \(error)")
