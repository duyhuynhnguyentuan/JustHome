//
//  ConfirmTransferContractOTPVerificationViewModel.swift
//  JustHome
//
...
class ConfirmTransferContractOTPVerificationViewModel: ObservableObject{
    var nowDate = Date()
    ...
    func startTimer() async throws {
        try await confirmContractSendOTP()
           timerExpired = false
    ...
    @MainActor
    func confirmContractSendOTP() async throws {
        do{
            _ = try await contractsService.checkTransferSendOTP(contractID: contractId)
    ...
    @MainActor
    func confirmContractVerifyOTP() async throws -> MessageResponse{
        defer { loadingState = .finished }
        do {
            loadingState = .loading
            let response = try await contractsService.checkTransferVerifyOTP(contractID: contractId, otp: verificationCode)
            return response
        }catch let error as NetworkError {
            self.error = error
            print("Error: \(error)")
